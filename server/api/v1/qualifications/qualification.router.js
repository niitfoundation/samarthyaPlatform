...
router.get('/academictypes', function (req, res) {
    try {
        qualificationCtrl.findAcademicTypes().then((successResult) => {
            return res.status(201).send(successResult);
        }, (errResult) => {
            // Log the error for internal use
            logger.error('Internal error occurred');
...
router.get('/subjects', function (req, res) {
    try {
        qualificationCtrl.findSubjects().then((successResult) => {
            return res.status(201).send(successResult);
        }, (errResult) => {
            // Log the error for internal use
            logger.error('Internal error occurred');
